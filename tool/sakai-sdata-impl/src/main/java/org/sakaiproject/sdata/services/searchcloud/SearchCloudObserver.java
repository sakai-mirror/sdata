 ...
 * @author
 */
public class SearchCloudObserver implements Observer
{

	private static final Log log = LogFactory.getLog(SearchCloudObserver.class);
	
	public SearchCloudObserver() {
		
		SqlService sqlService = Kernel.sqlService();
		boolean autoDDL = Kernel.serverConfigurationService().getBoolean("auto.ddl", false);
		if (autoDDL) {
			sqlService.ddl(this.getClass().getClassLoader(), "sdata_searchcloud");
		}
