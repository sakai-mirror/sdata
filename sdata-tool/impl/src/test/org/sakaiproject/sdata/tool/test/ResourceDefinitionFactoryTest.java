 ...
 * @author ieb
 */
public class ResourceDefinitionFactoryTest extends TestCase
{
	...
	public void testCreation()
	{
		...
		{
			ResourceDefinitionFactory rdf = new ResourceDefinitionFactory(basePath);
			for (String testPath : testPaths)
			{
				ResourceDefinition rd = rdf.getSpec(testPath);
				/*
				...
				assertTrue("Repository Paths must not end in /, except when root ", rp
						.equals("/")
						|| !rp.endsWith("/"));
				assertTrue("Repository Paths must not have white space at either end ",
				...
				assertTrue("External Paths must not have white space at either end ", rp
				...
				assertTrue("Extra Repository Paths must be absolute ", rp.startsWith("/"));
				assertTrue("Extra Repository Paths must not end in /, except when root ",
						rp.equals("/") || !rp.endsWith("/"));
				assertTrue(
						"Extra Repository Paths must not have white space at either end ",
						rp.length() == rp.trim().length());
				assertTrue("Extra Repository Patsh must no have // ",
						rp.indexOf("//") < 0);
